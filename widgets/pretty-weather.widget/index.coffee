apiKey: 'ad84eef669915a95c813439a07e76864' # put your forcast.io api key inside the quotes here

refreshFrequency: 300000 # Changed from updating every minute to updating every 5 minutes
...
update: (output, domEl) ->
  ...
  $(domEl).find('.temp').prop 'textContent', Math.round(curr.temperature)+'°'
  $(domEl).find('.summary').text today.summary
  $(domEl).find('.icon')[0]?.textContent = @getIcon(curr)
